...
{
	/// <summary>
	/// NumericGenerator is a simple random number generator based on 
	...
	/// 
	/// Modified by Patrick El-Azem to add the functionality in GeneratorBase as well as a GetUniform() overload and a couple of other minor tweaks.
	/// </summary>
	public class NumericGenerator : GeneratorBase<double>
	{
		...
		private void SetSeed(uint u, uint v)
		{
			if (u != 0) m_w = u;
			if (v != 0) m_z = v;
		}

		private void SetSeed(uint u)
		{
			m_w = u;
		}
		...
		{
			return (Convert.ToInt32(GetUniform(1000000, 2000000)) % 2) == 0;
		}
	}
}
